...
// 3. Rotas de sessão são sempre mascaradas para '/room', sem IDs da sala e sem tokens.
 ...
 * Garante que rotas de room não vazem o ID da sala nem o token do facilitador.
 */
  ...
  try {
    if (typeof window !== 'undefined' && window.umami) {
      window.umami.track((props) => ({
        ...props,
        url: sanitizedPath,
        title: title || (sanitizedPath === '/' ? 'PlanningYrd — Início' : 'PlanningYrd — Sala de Votação'),
      }));
    }
  } catch (err) {
