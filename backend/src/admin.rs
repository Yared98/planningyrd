...
use crate::models::{AdminMetrics, AdminRoomSummary};
...
pub fn get_configured_admin_token() -> String {
    std::env::var("ADMIN_TOKEN").unwrap_or_else(|_| {
        "dev_admin_planningyrd_secret".to_string()
    })
}

/// Valida se a requisição possui credencial administrativa válida
/// (aceita cookie HttpOnly ou cabeçalho Authorization: Bearer <token>)
...
async fn login_handler(
        ...
        if attempts >= RATE_LIMIT_MAX_ATTEMPTS {
            if last_attempt.elapsed() < Duration::from_secs(RATE_LIMIT_WINDOW_SECS) {
                ...
                warn!(
                    ...
                    "Admin login bloqueado temporariamente por rate limit"
                );
                return Err((
    ...
    if !is_valid {
        ...
        warn!(client_ip = %client_ip, "Tentativa de login administrativo com token inválido");
    ...
    info!(client_ip = %client_ip, "Sessão de administrador iniciada com sucesso");
...
async fn metrics_handler(
    ...
    let metrics = state.db.get_admin_metrics().map_err(|e| {
        tracing::error!(error = %e, "Erro ao obter métricas de admin");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    let rooms = state.db.list_admin_rooms(50).map_err(|e| {
        tracing::error!(error = %e, "Erro ao listar salas para admin");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    let active_rooms_memory = state.rooms.len();
...
async fn purge_handler(
    ...
    let purged_count = state.db.cleanup_expired_rooms(retention_days).map_err(|e| {
        tracing::error!(error = %e, "Erro ao executar expurgo manual");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    info!(purged_count = purged_count, "Expurgo manual de salas executado via Admin");
...
async fn delete_room_handler(
    ...
    let count = state.db.delete_room(&room_id).map_err(|e| {
        tracing::error!(error = %e, "Erro ao deletar sala via admin");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    state.rooms.remove(&room_id);
    info!(room_id = %room_id, "Sala removida pelo administrador");
